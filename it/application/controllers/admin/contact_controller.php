...
class contact_controller extends CI_Controller {
	...
	public function index()
	{
		 ...
		 $dulieu = $this->Contact_model->getdatabase();
		 $dulieu = array('dulieutucontroller'=> $dulieu);
		$this->session->set_flashdata('message');
	 	//lay noi dung của biến message
	 	$message = $this->session->flashdata('message');
	 	$this->data['message'] = $message; //$message la noi dung thong bao ta se truyen qua view
		$this->load->view('admin/contact_view',$dulieu,false);
	...
	{
		$this->load->model('admin/Contact_model');
		if ($this->Contact_model->deleteDataById($idnhanduoc)) 
		{
		...
		redirect(base_url('admin/contact_controller')); 	
	}
